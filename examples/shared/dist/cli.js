...
export function createBaseProgram(name, description) {
    ...
    return program
        ...
        .requiredOption("-u, --external_user_id <extuid>", "External user ID (required)")
        .option("-m, --model <model>", "OpenAI model to use", "gpt-4-1106-preview")
        .option("-s, --max-steps <steps>", "Maximum conversation steps", "10");
}
export function validateAndParseOptions(instruction, options) {
...
export async function createMCPTransport(external_user_id) {
...
export function logProcessingStart(config, sdkName) {
...
export function logMaxStepsReached(maxSteps) {
...
export function handleError(error, sdkName) {
